...
BaseApiWinRT::InitAsync(Platform::String^ datapath, Platform::String^ language, OcrEngineMode mode,
...
void BaseApiWinRT::GetBitmapLockFromWinRTStream(Windows::Storage::Streams::IRandomAccessStream ^ inputImage, CComPtr<IWICBitmapLock> & m_pBitmapLock, UINT & imageWidth, UINT & imageHeight)
{
...
Windows::Foundation::IAsyncOperation<Platform::String^>^ BaseApiWinRT::TesseractRectAsync(Windows::Storage::Streams::IRandomAccessStream ^ inputImage, Windows::Foundation::Rect rect)
{
	...
	return Concurrency::create_async([=]() -> Platform::String^
	{
		try
		{
			...
			GetBitmapLockFromWinRTStream(inputImage, m_pBitmapLock, imageWidth, imageHeight);
			...
			auto resultStr = baseAPI->TesseractRect(imageData, bytes_per_pixel, bytes_per_line, left, top, min(width, imageWidth), min(height, imageHeight) );
...
void BaseApiWinRT::SetImage(Windows::Storage::Streams::IRandomAccessStream ^ inputImage)
{
	try
	{
		unsigned char * imageData;
		...
		baseAPI->SetImage(imageData, imageWidth, imageHeight, bytes_per_pixel, bytes_per_line);
	}
	catch (HRESULT hr)
	{
		throw ref new Platform::Exception(hr, "TesseractRectAsync failed reading the input image");
...
Windows::Foundation::IAsyncOperation<Platform::String^>^ BaseApiWinRT::GetUTF8TextAsync()
{
	return Concurrency::create_async([=]() -> Platform::String^
	{
		USES_CONVERSION;
		
		auto resultStr = baseAPI->GetUTF8Text();
		Platform::String^ result = nullptr;
		if (resultStr)
		{
			result = resultStr ? ref new Platform::String(A2W_CP(resultStr, CP_UTF8)) : nullptr;
...
void BaseApiWinRT::SetSourceResolution(int ppi)
{
	baseAPI->SetSourceResolution(ppi);
}

void BaseApiWinRT::SetRectangle(int left, int top, int width, int height)
{
	baseAPI->SetRectangle(left, top, width, height);
}

void BaseApiWinRT::Clear()
{
	baseAPI->Clear();
}

void BaseApiWinRT::End()
{
	baseAPI->End();
}
